...
EC_BOOL crfshttps_log_start()
{
    ...
    if(EC_TRUE == task_brd_check_is_work_tcid(TASK_BRD_TCID(task_brd)))
    {
        ...
        {
            dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:crfshttps_log_start: user_log_open '%s' -> LOGUSER08 failed\n",
    ...
    if(EC_TRUE == task_brd_check_is_work_tcid(TASK_BRD_TCID(task_brd)))
    {
        ...
        if(NULL_PTR == log)
        {
            dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:crfshttps_log_start: log_file_open '%s' -> LOGUSER08 failed\n",
                               (char *)cstring_get_str(log_file_name));
            cstring_free(log_file_name);
            /*task_brd_default_abort();*/
        }
        else
        {
            ...
            dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "[DEBUG] crfshttps_log_start: log_file_open '%s' -> LOGUSER08 done\n",
                               (char *)cstring_get_str(log_file_name));
    ...
    if(EC_TRUE == task_brd_check_is_work_tcid(TASK_BRD_TCID(task_brd)))
    {
        ...
        if(NULL_PTR == log)
        {
            dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:crfshttps_log_start: log_file_open '%s' -> LOGUSER07 failed\n",
                               (char *)cstring_get_str(log_file_name));
            cstring_free(log_file_name);
            /*task_brd_default_abort();*/
        }
...
EC_BOOL crfshttps_commit_response(CHTTPS_NODE *chttps_node)
{
    ...
    ret = chttps_csocket_cnode_send_rsp(csocket_cnode);
    if(EC_AGAIN != ret)
    {
        return (ret);
    }
