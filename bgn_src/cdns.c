...
#if 1
#define CDNS_ASSERT(condition) do{\
    if(!(condition)) {\
...
EC_BOOL cdns_node_init(CDNS_NODE *cdns_node)
{
    if(NULL_PTR != cdns_node)
    {
        ...
        //CDNS_NODE_CQUEUE_DATA(cdns_node)       = NULL_PTR;
...
EC_BOOL cdns_node_clean(CDNS_NODE *cdns_node)
{
    if(NULL_PTR != cdns_node)
    {
        ...
        //CDNS_NODE_CQUEUE_DATA(cdns_node)       = NULL_PTR; /*already umount cdns_node from defer list*/
...
EC_BOOL cdns_csocket_cnode_init(const UINT32 UNUSED(md_id), CSOCKET_CNODE *csocket_cnode)
{
    CDNS_NODE    *cdns_node;
    ...
    if(NULL_PTR == cdns_node)
    {
        dbg_log(SEC_0150_CDNS, 0)(LOGSTDOUT, "error:cdns_csocket_cnode_init: sockfd %d -> cdns_node is null\n",
                        CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);
    }
...
void cdns_csocket_cnode_close(CSOCKET_CNODE *csocket_cnode)
{
    CDNS_NODE *cdns_node;
    int sockfd;
    ...
    if(NULL_PTR != CDNS_NODE_CROUTINE_COND(cdns_node) && BIT_FALSE == CDNS_NODE_COROUTINE_RESTORE(cdns_node))
    {
        dbg_log(SEC_0150_CDNS, 9)(LOGSTDOUT, "[DEBUG] cdns_csocket_cnode_close: socket %d retore coroutine %p\n", sockfd, CDNS_NODE_CROUTINE_COND(cdns_node));
    ...
    csocket_cnode_close(csocket_cnode);
    csocket_cnode_stat_dec();
    dbg_log(SEC_0150_CDNS, 9)(LOGSTDOUT, "[DEBUG] cdns_csocket_cnode_close: release cdns_node and  try to close socket %d done\n", sockfd);
    return;
}
...
EC_BOOL cdns_req_init_0(const UINT32 UNUSED(md_id), CDNS_REQ *cdns_req)
{
    return cdns_req_init(cdns_req);
}

EC_BOOL cdns_req_clean_0(const UINT32 UNUSED(md_id), CDNS_REQ *cdns_req)
{
    return cdns_req_clean(cdns_req);
}

EC_BOOL cdns_req_free_0(const UINT32 UNUSED(md_id), CDNS_REQ *cdns_req)
{
    return cdns_req_free(cdns_req);
}
...
EC_BOOL cdns_rsp_init_0(const UINT32 UNUSED(md_id), CDNS_RSP *cdns_rsp)
{
    return cdns_rsp_init(cdns_rsp);
}

EC_BOOL cdns_rsp_clean_0(const UINT32 UNUSED(md_id), CDNS_RSP *cdns_rsp)
{
    return cdns_rsp_clean(cdns_rsp);
}

EC_BOOL cdns_rsp_free_0(const UINT32 UNUSED(md_id), CDNS_RSP *cdns_rsp)
{
    return cdns_rsp_free(cdns_rsp);
}
...
EC_BOOL cdns_rsp_node_init_0(const UINT32 UNUSED(md_id), CDNS_RSP_NODE *cdns_rsp_node)
{
    return cdns_rsp_node_init(cdns_rsp_node);
}

EC_BOOL cdns_rsp_node_clean_0(const UINT32 UNUSED(md_id), CDNS_RSP_NODE *cdns_rsp_node)
{
    return cdns_rsp_node_clean(cdns_rsp_node);
}

EC_BOOL cdns_rsp_node_free_0(const UINT32 UNUSED(md_id), CDNS_RSP_NODE *cdns_rsp_node)
{
    return cdns_rsp_node_free(cdns_rsp_node);
}
...
EC_BOOL cdns_node_create(CDNS_NODE *cdns_node, const CDNS_REQ * cdns_req)
{
    ...
    CSOCKET_CNODE_CDNS_NODE(csocket_cnode) = cdns_node;
                 
    ret = cdns_csocket_cnode_send_req(csocket_cnode);
    if(EC_FALSE == ret)
    {
    ...
    if(EC_AGAIN == ret)
    {
        ...
        cepoll_set_event(task_brd_default_get_cepoll(),
                        CSOCKET_CNODE_SOCKFD(csocket_cnode),
                        CEPOLL_WR_EVENT,
                        (CEPOLL_EVENT_HANDLER)cdns_csocket_cnode_send_req,
                        (void *)csocket_cnode);

        cepoll_set_complete(task_brd_default_get_cepoll(),
                        CSOCKET_CNODE_SOCKFD(csocket_cnode),
                        (CEPOLL_EVENT_HANDLER)cdns_csocket_cnode_close,
                        (void *)csocket_cnode);
    ...
    cepoll_set_event(task_brd_default_get_cepoll(), CSOCKET_CNODE_SOCKFD(csocket_cnode), CEPOLL_RD_EVENT,
                     (CEPOLL_EVENT_HANDLER)cdns_csocket_cnode_recv_rsp, (void *)csocket_cnode);
...
EC_BOOL cdns_csocket_cnode_send_req(CSOCKET_CNODE *csocket_cnode)
{
    CDNS_NODE *cdns_node;
    ...
    if(NULL_PTR == cdns_node)
    {
        dbg_log(SEC_0150_CDNS, 0)(LOGSTDOUT, "error:cdns_csocket_cnode_send_req: sockfd %d -> cdns_node is null\n",
                       CSOCKET_CNODE_SOCKFD(csocket_cnode));
    ...
    {
        dbg_log(SEC_0150_CDNS, 0)(LOGSTDOUT, "error:cdns_csocket_cnode_send_req: sockfd %d send req failed\n",
                       CSOCKET_CNODE_SOCKFD(csocket_cnode));
    ...
    dbg_log(SEC_0150_CDNS, 9)(LOGSTDOUT, "[DEBUG] cdns_csocket_cnode_send_req: sockfd %d had sent out all req data\n",
                       CSOCKET_CNODE_SOCKFD(csocket_cnode));  
    return (EC_TRUE);
}

EC_BOOL cdns_csocket_cnode_recv_rsp(CSOCKET_CNODE *csocket_cnode)
{
    CDNS_NODE  *cdns_node;
    EC_BOOL     ret;
    ...
    if(NULL_PTR == cdns_node)
    {
        dbg_log(SEC_0150_CDNS, 0)(LOGSTDOUT, "error:cdns_csocket_cnode_recv_rsp: sockfd %d -> cdns_node is null\n",
                           CSOCKET_CNODE_SOCKFD(csocket_cnode));
    ...
    if(EC_FALSE == ret)
    {
        dbg_log(SEC_0150_CDNS, 0)(LOGSTDOUT, "error:cdns_csocket_cnode_recv_rsp: recv req on sockfd %d failed\n",
                            CSOCKET_CNODE_SOCKFD(csocket_cnode));
    ...
    if(EC_DONE == ret)
    {
        dbg_log(SEC_0150_CDNS, 9)(LOGSTDOUT, "[DEBUG] cdns_csocket_cnode_recv_rsp: sockfd %d, no more data to recv or parse\n",
                CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_DONE);
    }
    ...
    {
        dbg_log(SEC_0150_CDNS, 0)(LOGSTDOUT, "error:cdns_csocket_cnode_recv_rsp: parse on sockfd %d failed\n",
                            CSOCKET_CNODE_SOCKFD(csocket_cnode));

        return (EC_FALSE);                         
    }

    dbg_log(SEC_0150_CDNS, 9)(LOGSTDOUT, "[DEBUG] cdns_csocket_cnode_recv_rsp: sockfd %d, recv and parse done\n",
            CSOCKET_CNODE_SOCKFD(csocket_cnode));
...
EC_BOOL cdns_request(const CDNS_REQ *cdns_req, CDNS_RSP *cdns_rsp)
{
    ...
    __COROUTINE_CATCH_EXCEPTION() { /*exception*/
        ...
        {
            cepoll_del_all(task_brd_default_get_cepoll(), CSOCKET_CNODE_SOCKFD(CDNS_NODE_CSOCKET_CNODE(cdns_node)));
        }
    ...
    if(NULL_PTR != CDNS_NODE_CSOCKET_CNODE(cdns_node))
    {
        ...
        /*unbind csocket_cnode and cdns_node*/
        CSOCKET_CNODE_CDNS_NODE(csocket_cnode) = NULL_PTR;
        CDNS_NODE_CSOCKET_CNODE(cdns_node)     = NULL_PTR;
    ...
    /*unbind cdns_req and cdns_rsp from cdns_node*/
    CDNS_NODE_REQ(cdns_node) = NULL_PTR;
    CDNS_NODE_RSP(cdns_node) = NULL_PTR; 
