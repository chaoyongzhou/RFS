...
//#define CSTRKV_DEBUG(__X)   __X
#define CSTRKV_DEBUG(__X)   do{}while(0)
...
{
    if(0 == cstring_ignore_case_cmp(CSTRKV_KEY(cstrkv_1), CSTRKV_KEY(cstrkv_2)))
    {
        return (EC_TRUE);
    }
    return (EC_FALSE);
...
{
    if(0 == cstring_ignore_case_cmp(CSTRKV_VAL(cstrkv_1), CSTRKV_VAL(cstrkv_2)))
    {
        return (EC_TRUE);
    }
    return (EC_FALSE);
...
{
    CSTRKV_DEBUG(dbg_log(SEC_0008_CSTRKV, 0)(LOGSTDOUT, "[DEBUG]__cstrkv_free: %p (key %p, val %p), (klen %ld, vlen %ld)\n",
                cstrkv,
                CSTRKV_KEY_STR(cstrkv), CSTRKV_VAL_STR(cstrkv),
                CSTRKV_KEY_LEN(cstrkv), CSTRKV_VAL_LEN(cstrkv)
                ));
...
EC_BOOL cstrkv_mgr_add_kv(CSTRKV_MGR *cstrkv_mgr, const CSTRKV *cstrkv)
{
    ...
    CSTRKV_DEBUG(dbg_log(SEC_0008_CSTRKV, 0)(LOGSTDOUT, "[DEBUG]cstrkv_mgr_add_kv: %p (key %p, val %p), (klen %ld, vlen %ld)\n",
                cstrkv,
                CSTRKV_KEY_STR(cstrkv), CSTRKV_VAL_STR(cstrkv),
                CSTRKV_KEY_LEN(cstrkv), CSTRKV_VAL_LEN(cstrkv)
                ));
...
EC_BOOL cstrkv_mgr_exit_kv(const CSTRKV_MGR *cstrkv_mgr, const char *key, const char *val)
{
    char *v;
...
EC_BOOL cstrkv_mgr_clone(CSTRKV_MGR *cstrkv_mgr_des, const CSTRKV_MGR *cstrkv_mgr_src)
{
