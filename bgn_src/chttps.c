...
static EC_BOOL __chttps_on_recv_complete(CHTTPS_NODE *chttps_node)
{
    ...
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:__chttps_on_recv_complete: chttps_node %p -> csocket_cnode is null\n", chttps_node);
    ...
    if(CHTTP_TYPE_HANDLE_REQ == CHTTPS_NODE_TYPE(chttps_node))
    {
    ...
    if(CHTTP_TYPE_HANDLE_RSP == CHTTPS_NODE_TYPE(chttps_node))
    {
    ...
    if(CHTTP_TYPE_HANDLE_CHECK == CHTTPS_NODE_TYPE(chttps_node))
    {
...
static EC_BOOL __chttps_on_send_complete(CHTTPS_NODE *chttps_node)
{
    ...
    if(NULL_PTR == csocket_cnode)
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:__chttps_on_send_complete: chttps_node %p -> csocket_cnode is null\n", chttps_node);
        return (EC_FALSE);/*error*/
    } 
...
EC_BOOL chttps_node_clean(CHTTPS_NODE *chttps_node)
{
    if(NULL_PTR != chttps_node)
    {
        ...
        CHTTPS_NODE_TYPE(chttps_node)        = CHTTP_TYPE_HANDLE_ERR;
        CHTTPS_NODE_STATUS_CODE(chttps_node) = CHTTP_STATUS_NONE;
...
EC_BOOL chttps_parse_content_length(CHTTPS_NODE *chttps_node)
{
    ...
    CHTTPS_NODE_CONTENT_LENGTH(chttps_node) = c_chars_to_uint64((char *)cstring_get_str(content_length_cstr),
...
EC_BOOL chttps_make_response_header_expires(CHTTPS_NODE *chttps_node)
{
    ...
    {
        //dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] chttps_make_response_header_expires: [%.*s] len = %u\n", CBUFFER_USED(expires), (char *)CBUFFER_DATA(expires), CBUFFER_USED(expires));
...
EC_BOOL chttps_srv_accept_once(CSRV *csrv, EC_BOOL *continue_flag)
{
    ...
    if(EC_TRUE == ret)
    {
        ...
        chttps_node = chttps_node_new(CHTTP_TYPE_HANDLE_REQ);
        if(NULL_PTR == chttps_node)
        {
        ...
        /*bind csocket_cnode and chttps_node*/
        CSOCKET_CNODE_CHTTPS_NODE(csocket_cnode) = chttps_node;
        ...
        if(NULL_PTR != CSRV_INIT_CSOCKET_CNODE(csrv))
        {
            ...
            {
                dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_srv_accept_once: csrv init sockfd %d failed\n", client_conn_sockfd);

                /*unbind csocket_cnode and chttps_node*/
        ...
        /*shakehand with server*/
        cepoll_set_event(task_brd_default_get_cepoll(),
                          CSOCKET_CNODE_SOCKFD(csocket_cnode),
                          CEPOLL_RD_EVENT,
                          (CEPOLL_EVENT_HANDLER)chttps_csocket_cnode_handshake_on_server,
                          (void *)csocket_cnode);
...
EC_BOOL chttps_commit_error_response(CHTTPS_NODE *chttps_node)
{
    ...
    return cepoll_set_event(task_brd_default_get_cepoll(), CSOCKET_CNODE_SOCKFD(csocket_cnode), CEPOLL_WR_EVENT,
                 (CEPOLL_EVENT_HANDLER)chttps_csocket_cnode_send_rsp, csocket_cnode);
}
...
EC_BOOL chttps_csocket_cnode_init(const UINT32 UNUSED(md_id), CSOCKET_CNODE *csocket_cnode)
{
    ...
    if(NULL_PTR == chttps_node)
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_init: sockfd %d -> chttps_node is null\n",
                        CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);
    }
...
EC_BOOL chttps_csocket_cnode_close(CSOCKET_CNODE *csocket_cnode)
{
    ...
    if(NULL_PTR == chttps_node)
    {
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_close: no chttps_node, force to close socket %d\n", sockfd);
        csocket_cnode_force_close(csocket_cnode);
        csocket_cnode_stat_dec();
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_close: no chttps_node, force to close socket %d done\n", sockfd);
        return (EC_TRUE); 
    }
 
    if(CHTTP_TYPE_HANDLE_REQ == CHTTPS_NODE_TYPE(chttps_node))/*on server side*/
    {
        ...
        if(BIT_TRUE == CHTTPS_NODE_KEEPALIVE(chttps_node))
        {
            dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_close: [server] keep-alive, resume socket %d\n", sockfd);
            /*resume*/
            chttps_node_wait_resume(chttps_node);
            dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_close: [server] keep-alive, resume socket %d done\n", sockfd);
        ...
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_close: [server] not keep-alive, force to close socket %d\n", sockfd);
        csocket_cnode_force_close(csocket_cnode);
        csocket_cnode_stat_dec();
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_close: [server] not keep-alive, force to close socket %d done\n", sockfd);

        return (EC_TRUE);
    }

    if(CHTTP_TYPE_HANDLE_RSP == CHTTPS_NODE_TYPE(chttps_node))/*on client side*/
    {
        /* unbind */
        CSOCKET_CNODE_CHTTPS_NODE(csocket_cnode) = NULL_PTR;
        CHTTPS_NODE_CSOCKET_CNODE(chttps_node)    = NULL_PTR;
        ...
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_close: has chttps_node, try to close socket %d\n", sockfd);
        csocket_cnode_try_close(csocket_cnode);
        csocket_cnode_stat_dec();
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_close: has chttps_node, try to close socket %d done\n", sockfd);
     
        return (EC_TRUE);
     
    }

    if(CHTTP_TYPE_HANDLE_CHECK == CHTTPS_NODE_TYPE(chttps_node))/*on client side*/
    {
        /*not unbind*/
    ...
    dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_close:should never reach here, release chttps_node and try to close socket %d\n", sockfd);
    /*free*/
    chttps_node_free(chttps_node);
   
    csocket_cnode_force_close(csocket_cnode);
    csocket_cnode_stat_dec();
    dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_close: should never reach here, release chttps_node and  try to close socket %d done\n", sockfd);
...
EC_BOOL chttps_csocket_cnode_force_close(CSOCKET_CNODE *csocket_cnode)
{
    ...
    if(NULL_PTR == chttps_node)
    {
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_force_close: no chttps_node, try to close socket %d\n", sockfd);
        csocket_cnode_force_close(csocket_cnode);
        csocket_cnode_stat_dec();
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_force_close: no chttps_node, try to close socket %d done\n", sockfd);
        return (EC_TRUE);
    }
    ...
    if(CHTTP_TYPE_HANDLE_REQ == CHTTPS_NODE_TYPE(chttps_node))
    {
        ...
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_force_close: released chttps_node, try to close socket %d\n", sockfd);
        csocket_cnode_force_close(csocket_cnode);
        csocket_cnode_stat_dec();
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_force_close: released chttps_node, try to close socket %d done\n", sockfd);     
    ...
    if(CHTTP_TYPE_HANDLE_RSP == CHTTPS_NODE_TYPE(chttps_node))
    {
        ...
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_force_close: has chttps_node, try to close socket %d\n", sockfd);
        csocket_cnode_force_close(csocket_cnode);
        csocket_cnode_stat_dec();
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_force_close: has chttps_node, try to close socket %d done\n", sockfd);
        return (EC_TRUE);
    ...
    dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_force_close:should never reach here, release chttps_node and try to close socket %d\n", sockfd);
    /*free*/
    chttps_node_free(chttps_node);
 
    csocket_cnode_force_close(csocket_cnode);
    csocket_cnode_stat_dec();
    dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_force_close: should never reach here, release chttps_node and  try to close socket %d done\n", sockfd);
    return (EC_TRUE);
}
...
EC_BOOL chttps_csocket_cnode_recv_req(CSOCKET_CNODE *csocket_cnode)
{
    ...
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_recv_req: sockfd %d is not connected\n",
                           CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);
    }
    ...
    if(NULL_PTR == chttps_node)
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_recv_req: sockfd %d -> chttps_node is null\n",
                           CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);
    }
    ...
    if(EC_FALSE == ret)
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_recv_req: recv req on sockfd %d failed\n",
                            CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);                         
    }
    ...
    if(EC_DONE == ret)
    {
        ...
        {
            dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_recv_req: sockfd %d, recv not completed => false\n",
                                CSOCKET_CNODE_SOCKFD(csocket_cnode));
     
            return (EC_FALSE);
        }
     
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_recv_req: sockfd %d, no more data to recv or parse\n",
                            CSOCKET_CNODE_SOCKFD(csocket_cnode));
    ...
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_recv_req: parse on sockfd %d failed\n",
                            CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);                         
    }

    dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_recv_req: sockfd %d, recv and parse done\n",
                            CSOCKET_CNODE_SOCKFD(csocket_cnode));
    return (EC_TRUE);
}
...
EC_BOOL chttps_csocket_cnode_send_rsp(CSOCKET_CNODE *csocket_cnode)
{
    ...
    if(EC_FALSE == CSOCKET_CNODE_IS_CONNECTED(csocket_cnode))
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_send_rsp: sockfd %d is not connected\n",
                           CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);
    }
    ...
    if(NULL_PTR == chttps_node)
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_send_rsp: sockfd %d -> chttps_node is null\n",
                       CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);
    }
    ...
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_send_rsp: sockfd %d send rsp failed\n",
                       CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);
    }
    ...
    dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_send_rsp: sockfd %d had sent out all rsp data\n",
                       CSOCKET_CNODE_SOCKFD(csocket_cnode));  
...
CHTTPS_REST *chttps_rest_new()
{
    ...
    alloc_static_mem(MM_CHTTPS_REST, &chttps_rest, LOC_CHTTPS_0013);
...
EC_BOOL chttps_rest_free(CHTTPS_REST *chttps_rest)
{
    if(NULL_PTR != chttps_rest)
    {
        ...
        free_static_mem(MM_CHTTPS_REST, chttps_rest, LOC_CHTTPS_0014);
    }
...
EC_BOOL chttps_rest_list_push(const char *name, EC_BOOL (*commit)(CHTTPS_NODE *))
{
    ...
    {
        g_chttps_rest_list = clist_new(MM_CHTTPS_REST, LOC_CHTTPS_0015);
        if(NULL_PTR == g_chttps_rest_list)
        {
...
EC_BOOL chttps_defer_request_queue_init()
{
    ...
    {
        cqueue_init(&g_chttps_defer_request_queue, MM_CHTTPS_NODE, LOC_CHTTPS_0016);
...
EC_BOOL chttps_defer_request_commit(CHTTPS_NODE *chttps_node)
{
    ...
    if('/' != rest_name[ 0 ])
    {
        dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "error:chttps_defer_request_commit: invalid url '%.*s' [len %d]\n",
                        CBUFFER_USED(uri_cbuffer),
                        CBUFFER_DATA(uri_cbuffer),
                        CBUFFER_USED(uri_cbuffer));
        return (EC_FALSE);                    
    ...
    for(http_req_idx = 0; http_req_idx < http_req_max_num; http_req_idx ++)
    {
        ...
        if(EC_FALSE == ret)/*Oops! found unknown request, dicard it now*/
        {
            ...
            if(NULL_PTR == csocket_cnode)
            {
                ...
                dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT,
                        "error:chttps_defer_request_queue_launch: csocket_cnode in chttps_node %p is null\n",
                        chttps_node);         
            ...
            else
            {
                ...
                dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_defer_request_queue_launch: try to close socket %d\n", sockfd);

                CHTTPS_NODE_KEEPALIVE(chttps_node) = BIT_FALSE; /*force to close the https connection*/
                
                /*warning: do not remove cepoll_del_all due to chttps_csocket_cnode_close may be called out side of cepoll.c*/
                cepoll_del_all(task_brd_default_get_cepoll(), sockfd);/*del all before csocket_cnode closing*/
                chttps_csocket_cnode_close(csocket_cnode);/*hence we have to call cepoll_del_all in chttps_csocket_cnode_close*/
...
EC_BOOL chttps_node_connect(CHTTPS_NODE *chttps_node, const UINT32 ipaddr, const UINT32 port)
{
    ...
    cepoll_set_event(task_brd_default_get_cepoll(),
                    CSOCKET_CNODE_SOCKFD(csocket_cnode),
                    CEPOLL_WR_EVENT,
                    (CEPOLL_EVENT_HANDLER)chttps_csocket_cnode_handshake_on_client,
                    (void *)csocket_cnode);

    cepoll_set_complete(task_brd_default_get_cepoll(),
                    CSOCKET_CNODE_SOCKFD(csocket_cnode),
                    (CEPOLL_EVENT_HANDLER)chttps_csocket_cnode_close,
                    (void *)csocket_cnode);

    cepoll_set_shutdown(task_brd_default_get_cepoll(),
                    CSOCKET_CNODE_SOCKFD(csocket_cnode),
                    (CEPOLL_EVENT_HANDLER)chttps_csocket_cnode_force_close,
                    (void *)csocket_cnode);
    ...
    dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_connect: sockfd %d set event WR\n",
                    CSOCKET_CNODE_SOCKFD(csocket_cnode)); 
...
EC_BOOL chttps_csocket_cnode_handshake_on_client(CSOCKET_CNODE *csocket_cnode)
{
    ...
    if(EC_FALSE == csocket_cnode_is_connected(csocket_cnode))
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_handshake_on_client: sockfd %d is not connected\n",
                           CSOCKET_CNODE_SOCKFD(csocket_cnode)); 
    ...
    if(NULL_PTR == chttps_node)
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_handshake_on_client: sockfd %d, chttps_node is null\n",
                    CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);
    }
    ...
    if(NULL_PTR == CHTTPS_NODE_CSSL_NODE(chttps_node))
    {
        ...
        if(NULL_PTR == cssl_node)
        {
            dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_handshake_on_client: new cssl_node failed\n");
            return (EC_FALSE);
        }
        ...
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_handshake_on_client: sockfd %d, bind chttps_node and cssl_node done\n",
                    CSOCKET_CNODE_SOCKFD(csocket_cnode));
    }
    ...
    if(EC_FALSE == ret)
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_handshake_on_client: sockfd %d, ssl connect failed\n",
                    CSOCKET_CNODE_SOCKFD(csocket_cnode));
    ...
    if(EC_TRUE == ret)
    {
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_handshake_on_client: sockfd %d, ssl connect done\n",
                    CSOCKET_CNODE_SOCKFD(csocket_cnode));

        if(do_log(SEC_0157_CHTTPS, 9))
        {
            dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_handshake_on_client: certificate is\n");
        ...
        cepoll_set_event(task_brd_default_get_cepoll(),
                        CSOCKET_CNODE_SOCKFD(csocket_cnode),
                        CEPOLL_WR_EVENT,
                        (CEPOLL_EVENT_HANDLER)chttps_csocket_cnode_send_req,
                        (void *)csocket_cnode);
    ...
    dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_handshake_on_client: sockfd %d, ssl handshake return %ld\n",
...
EC_BOOL chttps_csocket_cnode_handshake_on_server(CSOCKET_CNODE *csocket_cnode)
{
    ...
    if(EC_FALSE == csocket_cnode_is_connected(csocket_cnode))
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_handshake_on_server: sockfd %d is not connected\n",
                           CSOCKET_CNODE_SOCKFD(csocket_cnode)); 
    ...
    if(NULL_PTR == chttps_node)
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_handshake_on_server: sockfd %d, chttps_node is null\n",
                    CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);
    }
    ...
    if(NULL_PTR == csrv)
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_handshake_on_server: sockfd %d, chttps_node => csrv is null\n",
                    CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);
    }
    ...
    if(NULL_PTR == CHTTPS_NODE_CSSL_NODE(chttps_node))
    {
        ...
        if(NULL_PTR == cssl_node)
        {
            dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_handshake_on_server: sockfd %d, make cssl_node on server failed\n",
                            CSOCKET_CNODE_SOCKFD(csocket_cnode));
        ...
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_handshake_on_server: sockfd %d, bind chttps_node and cssl_node done\n",
                    CSOCKET_CNODE_SOCKFD(csocket_cnode));
    }
    ...
    if(EC_FALSE == ret)
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_handshake_on_server: sockfd %d, ssl handshake failed\n",
                    CSOCKET_CNODE_SOCKFD(csocket_cnode));
    ...
    if(EC_TRUE == ret)
    {
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_handshake_on_server: sockfd %d, ssl handshake done\n",
                    CSOCKET_CNODE_SOCKFD(csocket_cnode));
    ...
    dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_handshake_on_server: sockfd %d, ssl handshake return %ld\n",
                CSOCKET_CNODE_SOCKFD(csocket_cnode), ret);
...
EC_BOOL chttps_csocket_cnode_send_req(CSOCKET_CNODE *csocket_cnode)
{
    ...
    if(EC_FALSE == csocket_cnode_is_connected(csocket_cnode))
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_send_req: sockfd %d is not connected\n",
                           CSOCKET_CNODE_SOCKFD(csocket_cnode)); 
    ...
    if(NULL_PTR == chttps_node)
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_send_req: sockfd %d -> chttps_node is null\n",
                       CSOCKET_CNODE_SOCKFD(csocket_cnode));
    ...
    if(EC_FALSE == chttps_node_send(chttps_node, csocket_cnode))
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_send_req: sockfd %d send req failed\n",
                       CSOCKET_CNODE_SOCKFD(csocket_cnode));
    ...
    /*now all data had been sent out, del WR event and set RD event*/
    dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_send_req: sockfd %d had sent out all req data\n",
                       CSOCKET_CNODE_SOCKFD(csocket_cnode));  

    cepoll_del_event(task_brd_default_get_cepoll(), CSOCKET_CNODE_SOCKFD(csocket_cnode), CEPOLL_WR_EVENT);

    cepoll_set_event(task_brd_default_get_cepoll(), CSOCKET_CNODE_SOCKFD(csocket_cnode), CEPOLL_RD_EVENT,
                     (CEPOLL_EVENT_HANDLER)chttps_csocket_cnode_recv_rsp, (void *)csocket_cnode);
...
EC_BOOL chttps_csocket_cnode_recv_rsp(CSOCKET_CNODE *csocket_cnode)
{
    ...
    if(EC_FALSE == CSOCKET_CNODE_IS_CONNECTED(csocket_cnode))
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_recv_rsp: sockfd %d is not connected\n",
                           CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);
    }
    ...
    if(NULL_PTR == chttps_node)
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_recv_rsp: sockfd %d -> chttps_node is null\n",
                           CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);
    }
    ...
    if(EC_FALSE == ret)
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_recv_rsp: recv rsp on sockfd %d failed\n",
                            CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);                         
    }
    ...
    if(EC_DONE == ret)
    {
        dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_recv_rsp: sockfd %d, no more data to recv or parse\n",
                            CSOCKET_CNODE_SOCKFD(csocket_cnode));
    ...
    if(EC_FALSE == chttps_parse(chttps_node))
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_recv_rsp: parse on sockfd %d failed\n",
                            CSOCKET_CNODE_SOCKFD(csocket_cnode));
        return (EC_FALSE);                         
    }

    dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_csocket_cnode_recv_rsp: sockfd %d, recv and parse done\n",
                            CSOCKET_CNODE_SOCKFD(csocket_cnode));
...
EC_BOOL chttps_request_basic(const CHTTP_REQ *chttp_req, CHTTP_RSP *chttp_rsp, CHTTP_STAT *chttp_stat)
{
    ...
    __COROUTINE_IF_EXCEPTION() {/*exception*/
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_request_basic: found exception, refuse to launch http\n");
        return (EC_FALSE);
    }

    chttps_node = chttps_node_new(CHTTP_TYPE_HANDLE_RSP);
    if(NULL_PTR == chttps_node)
    {
    ...
    croutine_cond = croutine_cond_new(0/*never timeout*/, LOC_CHTTPS_0017);
    ...
    csocket_cnode = CHTTPS_NODE_CSOCKET_CNODE(chttps_node);
    chttps_csocket_cnode_init(CMPI_ANY_MODI, csocket_cnode);
    ...
    croutine_cond_reserve(croutine_cond, 1, LOC_CHTTPS_0018);
    croutine_cond_wait(croutine_cond, LOC_CHTTPS_0019);
    ...
    __COROUTINE_IF_EXCEPTION() {/*exception*/
        ...
        {
            cepoll_del_all(task_brd_default_get_cepoll(), CSOCKET_CNODE_SOCKFD(CHTTPS_NODE_CSOCKET_CNODE(chttps_node)));
        }
     } else {/*normal*/
        /*do nothing*/
    }
    ...
    if(NULL_PTR != CHTTPS_NODE_CSOCKET_CNODE(chttps_node))
    {
        ...
        dbg_log(SEC_0157_CHTTPS, 5)(LOGSTDOUT, "[DEBUG] chttps_request_basic: try close socket %d\n", CSOCKET_CNODE_SOCKFD(csocket_cnode));
        /*close http connection*/
        csocket_cnode_try_close(csocket_cnode);
        csocket_cnode_stat_dec();   
    }
...
EC_BOOL chttps_csocket_cnode_check(CSOCKET_CNODE *csocket_cnode)
{
    ...
    if(EC_FALSE == CSOCKET_CNODE_IS_CONNECTED(csocket_cnode))
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_check: sockfd %d is not connected\n",
                           CSOCKET_CNODE_SOCKFD(csocket_cnode)); 
    ...
    if(NULL_PTR == chttps_node)
    {
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_csocket_cnode_check: sockfd %d -> chttps_node is null\n",
                       CSOCKET_CNODE_SOCKFD(csocket_cnode));
...
EC_BOOL chttps_node_check(CHTTPS_NODE *chttps_node, const UINT32 ipaddr, const UINT32 port)
{
    ...
    cepoll_set_event(task_brd_default_get_cepoll(),
                    CSOCKET_CNODE_SOCKFD(csocket_cnode),
                    CEPOLL_WR_EVENT,
                    (CEPOLL_EVENT_HANDLER)chttps_csocket_cnode_check,
                    (void *)csocket_cnode);
    ...
    dbg_log(SEC_0157_CHTTPS, 9)(LOGSTDOUT, "[DEBUG] chttps_node_check: sockfd %d set event WR\n",
                    CSOCKET_CNODE_SOCKFD(csocket_cnode)); 
    return (EC_TRUE);
}
...
EC_BOOL chttps_check(const CHTTP_REQ *chttp_req, CHTTP_STAT *chttp_stat)
{
    ...
    __COROUTINE_IF_EXCEPTION() {/*exception*/
        dbg_log(SEC_0157_CHTTPS, 0)(LOGSTDOUT, "error:chttps_check: found exception, refuse to launch http\n");
        return (EC_FALSE);
    }
 
    chttps_node = chttps_node_new(CHTTP_TYPE_HANDLE_CHECK);
    if(NULL_PTR == chttps_node)
    {
    ...
    croutine_cond = croutine_cond_new(0/*never timeout*/, LOC_CHTTPS_0020);
    if(NULL_PTR == croutine_cond)
    {
    ...
    croutine_cond_reserve(croutine_cond, 1, LOC_CHTTPS_0021);
    croutine_cond_wait(croutine_cond, LOC_CHTTPS_0022);
    ...
    if(NULL_PTR != CHTTPS_NODE_CSOCKET_CNODE(chttps_node))
    {
        ...
        dbg_log(SEC_0157_CHTTPS, 5)(LOGSTDOUT, "[DEBUG] chttps_check: try close socket %d\n", CSOCKET_CNODE_SOCKFD(csocket_cnode));
