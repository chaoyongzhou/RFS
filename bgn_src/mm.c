...
#if (SWITCH_ON == NGX_BGN_SWITCH)
    MM_MGR_DEF(MM_CNGX_RANGE                   ,"MM_CNGX_RANGE                   ",32        , sizeof(CNGX_RANGE)                 , LOC_MM_0206);
#endif/*(SWITCH_ON == NGX_BGN_SWITCH)*/
...
UINT32 destory_static_mem()
{
    ...
    for ( type = 0; type < MM_END; type ++ )
    {
        ...
        MAN_CLEAN_LOCK(pMan, LOC_MM_0228);/*clean lock*/
    }
...
{
    free_static_mem(MM_MM_MAN_OCCUPY_NODE, mm_man_occupy_node, LOC_MM_0231);
    return (0);
}
...
{
    free_static_mem(MM_MM_MAN_LOAD_NODE, mm_man_load_node, LOC_MM_0232);
    return (0);
}
