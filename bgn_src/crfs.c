...
static EC_BOOL __crfs_qlist_tree(CRFSNP_DIT_NODE *crfsnp_dit_node, CRFSNP *crfsnp, CRFSNP_ITEM *crfsnp_item, const uint32_t node_pos)
{
    ...
    {
        dbg_log(SEC_0081_CRFSNP, 0)(LOGSTDOUT, "error:__crfs_qlist_tree: item was not used\n");
        return (EC_FALSE);
    }
    ...
    if(CRFSNP_ITEM_FILE_IS_REG == CRFSNP_ITEM_DIR_FLAG(crfsnp_item))
    {
        ...
        {
            dbg_log(SEC_0081_CRFSNP, 0)(LOGSTDOUT, "error:__crfs_qlist_tree: new cstring failed\n");
            return (EC_FALSE);
        }
     
        if(EC_FALSE == cstack_walk(CRFSNP_DIT_NODE_STACK(crfsnp_dit_node), (void *)full_path, (CSTACK_DATA_DATA_WALKER)__crfs_cat_path))
        {
            dbg_log(SEC_0081_CRFSNP, 0)(LOGSTDOUT, "error:__crfs_qlist_tree: walk stack failed\n");
    ...
    dbg_log(SEC_0081_CRFSNP, 0)(LOGSTDOUT, "error:__crfs_file_expire: invalid item dflag %u at node pos %u\n",
...
EC_BOOL crfs_wait_file_owner_notify_over_bgn (CRFS_WAIT_FILE *crfs_wait_file, const UINT32 tag)
{
    if(EC_FALSE == clist_is_empty(CRFS_WAIT_FILE_OWNER_LIST(crfs_wait_file)))
    {
        ...
        for(;;)
        {
            ...
            task_p2p_inc(task_mgr, CMPI_ANY_MODI, mod_node, &ret, FI_super_cond_wakeup, ERR_MODULE_ID, tag, CRFS_WAIT_FILE_NAME(crfs_wait_file));
