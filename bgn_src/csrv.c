...
EC_BOOL csrv_select(CSRV *csrv, int *ret)
{
    ...
    fd_cset = safe_malloc(sizeof(FD_CSET), LOC_CSRV_0005);
    if(NULL_PTR == fd_cset)
    {
    ...
    safe_free(fd_cset, LOC_CSRV_0007);
    return (EC_TRUE);
}
...
EC_BOOL csrv_rsp_clean(struct _TASK_FUNC *task_rsp_func)
{
    ...
    if(e_dbg_void != func_addr_node->func_ret_type)
    {
        ...
        {
            free_static_mem(TYPE_CONV_ITEM_VAR_MM_TYPE(type_conv_item), (void *)(task_rsp_func->func_ret_val), LOC_CSRV_0010);
    ...
    for( para_idx = 0; para_idx < task_rsp_func->func_para_num; para_idx ++ )
    {
        ...
        if(EC_TRUE == TYPE_CONV_ITEM_VAR_POINTER_FLAG(type_conv_item) && 0 != func_para->para_val)
        {
            free_static_mem(TYPE_CONV_ITEM_VAR_MM_TYPE(type_conv_item), (void *)(func_para->para_val), LOC_CSRV_0011);
            func_para->para_val = 0;
        }
    }
