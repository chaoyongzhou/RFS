...
static const CRFSNGX_OP_DEF g_crfsngx_op_tab[ ] = {
    {(const uint8_t *)CRFSNGX_GET_OP_STR   ,  CRFSNGX_GET_OP_SIZE   , CRFSNGX_GET_TAG   },
    {(const uint8_t *)CRFSNGX_SET_OP_STR   ,  CRFSNGX_SET_OP_SIZE   , CRFSNGX_SET_TAG   },
    {(const uint8_t *)CRFSNGX_DEL_OP_STR   ,  CRFSNGX_DEL_OP_SIZE   , CRFSNGX_DEL_TAG   },
    {(const uint8_t *)CRFSNGX_SELECT_OP_STR,  CRFSNGX_SELECT_OP_SIZE, CRFSNGX_SELECT_TAG},
};
...
EC_BOOL crfsngx_csocket_fetch_ngx_op(CSOCKET_CNODE *csocket_cnode, TASK_NODE *task_node)
{
    ...
    op_tag = __crfsngx_csocket_fetch_ngx_op((char *)(TASK_NODE_BUFF(task_node) + offset), op_str_len + CRFSNGX_END_SIZE);
...
EC_BOOL crfsngx_csocket_fetch_ngx_vlen(CSOCKET_CNODE *csocket_cnode, TASK_NODE *task_node)
{
    ...
    vlen = c_chars_to_uint32((char *)(TASK_NODE_BUFF(task_node) + offset), vlen_str_len);
    if(0 == vlen)
    {
...
EC_BOOL crfsngx_handle_ngx_request(CSOCKET_CNODE *csocket_cnode, TASK_NODE *task_req_node, FUNC_ADDR_NODE *func_addr_node)
{
    ...
    cepoll_set_event(task_brd_default_get_cepoll(), CSOCKET_CNODE_SOCKFD(csocket_cnode), CEPOLL_WR_EVENT,
                      (CEPOLL_EVENT_HANDLER)crfsngx_send_on_csocket_cnode, csocket_cnode);
    return (EC_TRUE);
}
...
EC_BOOL crfsngx_commit_ngx_get_request(CSOCKET_CNODE *csocket_cnode, TASK_NODE *task_req_node)
{
    ...
    CROUTINE_NODE_COND_RELEASE(croutine_node, LOC_CRFSNGX_0005);
...
EC_BOOL crfsngx_commit_ngx_set_request(CSOCKET_CNODE *csocket_cnode, TASK_NODE *task_req_node)
{
    ...
    CROUTINE_NODE_COND_RELEASE(croutine_node, LOC_CRFSNGX_0006);
...
EC_BOOL crfsngx_commit_ngx_get_reply(CSOCKET_CNODE *csocket_cnode, TASK_NODE *task_req_node)
{
    ...
    task_rsp = task_rsp_new(0, LOC_CRFSNGX_0009);
    ...
    if(EC_FALSE == ret)
    {
        ...
        send_len = snprintf((char *)TASK_NODE_BUFF(task_rsp_node), TASK_NODE_BUFF_LEN(task_rsp_node), CRFSNGX_FAIL_STATUS_STR);
        TASK_NODE_BUFF_LEN(task_rsp_node) = send_len;

        crfsngx_clean_ngx_get_request(task_req_node);
        task_node_free(task_req_node);

        CSOCKET_CNODE_SENDING_TASK_NODE(csocket_cnode) = task_rsp_node;
        return (EC_TRUE);                         
    }
    ...
    send_len  = 0;
    send_len += snprintf((char *)TASK_NODE_BUFF(task_rsp_node) + send_len, TASK_NODE_BUFF_LEN(task_rsp_node) - send_len, CRFSNGX_SUCC_STATUS_STR);
    send_len += snprintf((char *)TASK_NODE_BUFF(task_rsp_node) + send_len, TASK_NODE_BUFF_LEN(task_rsp_node) - send_len, "%ld\r\n", file_len);
...
EC_BOOL crfsngx_commit_ngx_set_reply(CSOCKET_CNODE *csocket_cnode, TASK_NODE *task_req_node)
{
    ...
    task_rsp = task_rsp_new(0, LOC_CRFSNGX_0010);
    task_rsp_node = TASK_RSP_NODE(task_rsp);
...
EC_BOOL crfsngx_recv_on_csocket_cnode(CSOCKET_CNODE *csocket_cnode)
{
    ...
    for(;;)
    {
        ...
        task_node = task_node_new(CRFSNGX_CSOCKET_BUFF_LEN, LOC_CRFSNGX_0013);
        if(NULL_PTR == task_node)
        {
...
EC_BOOL crfsngx_send_on_csocket_cnode(CSOCKET_CNODE *csocket_cnode)
{
    ...
    if(NULL_PTR == CSOCKET_CNODE_SENDING_TASK_NODE(csocket_cnode))
    {
        ...
        cepoll_set_event(task_brd_default_get_cepoll(), CSOCKET_CNODE_SOCKFD(csocket_cnode), CEPOLL_RD_EVENT,
                          (CEPOLL_EVENT_HANDLER)crfsngx_recv_on_csocket_cnode, csocket_cnode);
