...
UINT32 super_excl_taskc_node(const UINT32 super_md_id, const UINT32 tcid, const UINT32 comm)
{
    ...
    CVECTOR_LOCK(tasks_node_work, LOC_SUPER_0020);
    ...
    CVECTOR_UNLOCK(tasks_node_work, LOC_SUPER_0021);
...
UINT32 super_sync_taskc_mgr(const UINT32 super_md_id, TASKC_MGR *des_taskc_mgr)
{
    ...
    CVECTOR_LOCK(tasks_node_work, LOC_SUPER_0022);
    ...
    CVECTOR_UNLOCK(tasks_node_work, LOC_SUPER_0023);
...
UINT32 super_sync_cload_mgr0(const UINT32 super_md_id, const UINT32 type, CLOAD_MGR *des_cload_mgr)
{
    ...
    CVECTOR_LOCK(tasks_node_work, LOC_SUPER_0024);
    for(tasks_node_pos = 0; tasks_node_pos < cvector_size(tasks_node_work); tasks_node_pos ++)
    {
        TASKS_NODE *tasks_node;
    ...
    CVECTOR_UNLOCK(tasks_node_work, LOC_SUPER_0025);
...
UINT32 super_sync_cload_mgr(const UINT32 super_md_id, const CVECTOR *tcid_vec, CLOAD_MGR *des_cload_mgr)
{
    ...
    CVECTOR_LOCK(tasks_node_work, LOC_SUPER_0026);
    for(tasks_node_pos = 0; tasks_node_pos < cvector_size(tasks_node_work); tasks_node_pos ++)
    {
        TASKS_NODE *tasks_node;
        CLOAD_NODE *cload_node;
    ...
    CVECTOR_UNLOCK(tasks_node_work, LOC_SUPER_0027);
...
void super_activate_sys_cfg(const UINT32 super_md_id)
{
    ...
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:super_activate_sys_cfg: shortcut config failed\n");
...
EC_BOOL super_cancel_task_req(const UINT32 super_md_id, const UINT32 seqno, const UINT32 subseqno, const MOD_NODE *recv_mod_node)
{
    ...
    TASK_NODE_CMUTEX_LOCK(TASK_REQ_NODE(task_req), LOC_SUPER_0028);
    TASK_NODE_STATUS(TASK_REQ_NODE(task_req)) = TASK_REQ_DISCARD;
    TASK_MGR_COUNTER_INC_BY_TASK_REQ(TASK_REQ_MGR(task_req), TASK_MGR_COUNTER_TASK_REQ_DISCARD, task_req, LOC_SUPER_0029);
    TASK_NODE_CMUTEX_UNLOCK(TASK_REQ_NODE(task_req), LOC_SUPER_0030);
...
static void super_sync_taskcomm_intranet(CVECTOR *collected_vec, CVECTOR *remote_tasks_cfg_vec, TASKS_CFG *local_tasks_cfg, const UINT32 max_hops, const UINT32 max_remotes, const UINT32 time_to_live, TASK_MGR *task_mgr, MOD_NODE *send_mod_node)
{
    ...
    CVECTOR_LOCK(remote_tasks_cfg_vec, LOC_SUPER_0032);
    for(pos = 0; pos < cvector_size(remote_tasks_cfg_vec); pos ++)
    {
        ...
        new_mod_node_vec = cvector_new(0, MM_MOD_NODE, LOC_SUPER_0033);
        cvector_push_no_lock(collected_vec, (void *)new_mod_node_vec);
    ...
    CVECTOR_UNLOCK(remote_tasks_cfg_vec, LOC_SUPER_0034);
...
static void super_sync_taskcomm_lannet(CVECTOR *collected_vec, CVECTOR *remote_tasks_cfg_vec, TASKS_CFG *local_tasks_cfg, const UINT32 max_hops, const UINT32 max_remotes, const UINT32 time_to_live, TASK_MGR *task_mgr, MOD_NODE *send_mod_node)
{
    ...
    CVECTOR_LOCK(remote_tasks_cfg_vec, LOC_SUPER_0035);
    for(pos = 0; pos < cvector_size(remote_tasks_cfg_vec); pos ++)
    {
        TASKS_CFG *remote_tasks_cfg;
        CVECTOR *new_mod_node_vec;
        ...
        new_mod_node_vec = cvector_new(0, MM_MOD_NODE, LOC_SUPER_0036);
        cvector_push_no_lock(collected_vec, (void *)new_mod_node_vec);
    ...
    CVECTOR_UNLOCK(remote_tasks_cfg_vec, LOC_SUPER_0037);
    return;
}
...
static void super_sync_taskcomm_externet(CVECTOR *collected_vec, CVECTOR *remote_tasks_cfg_vec, TASKS_CFG *local_tasks_cfg, const UINT32 max_hops, const UINT32 max_remotes, const UINT32 time_to_live, TASK_MGR *task_mgr, MOD_NODE *send_mod_node)
{
    ...
    CVECTOR_LOCK(remote_tasks_cfg_vec, LOC_SUPER_0038);
    for(pos = 0; pos < cvector_size(remote_tasks_cfg_vec); pos ++)
    {
        TASKS_CFG *remote_tasks_cfg;
        CVECTOR *new_mod_node_vec;
        ...
        new_mod_node_vec = cvector_new(0, MM_MOD_NODE, LOC_SUPER_0039);
        cvector_push_no_lock(collected_vec, (void *)new_mod_node_vec);
    ...
    CVECTOR_UNLOCK(remote_tasks_cfg_vec, LOC_SUPER_0040);
    return;
}
...
void super_sync_taskcomm(const UINT32 super_md_id, const UINT32 src_tcid, const UINT32 src_maski, const UINT32 src_maske, const UINT32 max_hops, const UINT32 max_remotes, const UINT32 time_to_live, CVECTOR *mod_node_vec)
{
    ...
    collected_vec = cvector_new(0, MM_CVECTOR, LOC_SUPER_0041);
    ...
    for(pos = 0; pos < cvector_size(collected_vec); pos ++)
    {
        ...
        //cvector_clean(new_mod_node_vec, (CVECTOR_DATA_CLEANER)mod_node_free, LOC_SUPER_0043);
        cvector_free(new_mod_node_vec, LOC_SUPER_0044);
    }

    /*when reach here, collected_vec should have no more element*/
    cvector_free(collected_vec, LOC_SUPER_0045);
...
void super_notify_broken_tcid(const UINT32 super_md_id, const UINT32 broken_tcid)
{
    ...
    /*pre-checking*/
    CVECTOR_LOCK(TASK_BRD_BROKEN_TCID_TBL(task_brd), LOC_SUPER_0046);
    ...
    CVECTOR_LOCK(TASK_BRD_BROKEN_TCID_TBL(task_brd), LOC_SUPER_0049);
    ...
    CVECTOR_UNLOCK(TASK_BRD_BROKEN_TCID_TBL(task_brd), LOC_SUPER_0050);
...
void super_register_cluster(const UINT32 super_md_id, const UINT32 src_tcid, const UINT32 broken_tcid)
{
    ...
    task_p2p_no_wait(super_md_id, TASK_DEFAULT_LIVE, TASK_PRIO_PREEMPT, TASK_NOT_NEED_RSP_FLAG, TASK_NEED_NONE_RSP,
                    &recv_mod_node,
                    NULL_PTR, FI_super_register_cluster, ERR_MODULE_ID, src_tcid, broken_tcid);
    return;
}
...
void super_show_route_table(const UINT32 super_md_id, LOG *log)
{
    ...
    CVECTOR_LOCK(TASKS_CFG_TASKR_CFG_VEC(tasks_cfg), LOC_SUPER_0051);
    ...
    CVECTOR_UNLOCK(TASKS_CFG_TASKR_CFG_VEC(tasks_cfg), LOC_SUPER_0053);
    return;
}
...
EC_BOOL super_http_request(const UINT32 super_md_id, const CHTTP_REQ *chttp_req, const CHTTP_STORE *chttp_store, CHTTP_RSP *chttp_rsp, CHTTP_STAT *chttp_stat)
{
    ...
    )
    {
        ...
        if(EC_FALSE == c_parse_location(loc, &host, &port, &uri))
        {
            if(NULL_PTR != host)
            {
                safe_free(host, LOC_SUPER_0054);
            }
            if(NULL_PTR != port)
            {
                safe_free(port, LOC_SUPER_0055);
            }
            if(NULL_PTR != uri)
            {
                safe_free(uri, LOC_SUPER_0056);
            }         
            break;
        }
...
void super_wait_me_ready(const UINT32 super_md_id)
{
    ...
    TASK_BRD_FWD_CCOND_WAIT(task_brd, LOC_SUPER_0060);
    return;
}
...
void super_run_shell(const UINT32 super_md_id, const CSTRING *cmd_line, LOG *log)
{
    ...
    result = cstring_new(NULL_PTR, LOC_SUPER_0061);
...
EC_BOOL super_exec_shell(const UINT32 super_md_id, const CSTRING *cmd_line, CBYTES *cbytes)
{
    ...
    cmd_line_fix = cstring_new(cstring_get_str(cmd_line), LOC_SUPER_0062);
...
EC_BOOL super_exec_shell_vec_ipaddr_cstr(const UINT32 super_md_id, const CVECTOR *ipaddr_cstr_vec, const CSTRING *cmd_line, CVECTOR *output_cbytes_vec)
{
    ...
    for(ipaddr_pos = 0; ipaddr_pos < cvector_size(ipaddr_cstr_vec); ipaddr_pos ++)
    {
        ...
        if(NULL_PTR == output_cbytes)
        {
            ...
            cvector_clean_no_lock(output_cbytes_vec, (CVECTOR_DATA_CLEANER)cbytes_free, LOC_SUPER_0065);
            task_mgr_free(task_mgr);
            return (EC_FALSE);
        }
...
EC_BOOL super_exec_shell_cbtimer_set(const UINT32 super_md_id, const CSTRING *cbtimer_name, const CSTRING *cmd_line, const UINT32 timeout)
{
    ...
    CBTIMER_NODE_NAME(cbtimer_node) = cstring_new(cstring_get_str(cbtimer_name), LOC_SUPER_0068);
    ...
    handler->func_para[ 0 ].para_val = super_md_id;
    handler->func_para[ 1 ].para_val = (UINT32)cstring_new(cstring_get_str(cmd_line), LOC_SUPER_0069);
    handler->func_para[ 2 ].para_val = (UINT32)cbytes_new(0);
...
EC_BOOL super_download(const UINT32 super_md_id, const CSTRING *fname, CBYTES *cbytes)
{
    ...
    fbuf = (UINT8 *)SAFE_MALLOC(fsize, LOC_SUPER_0070);
    if(NULL_PTR == fbuf)
    {
...
EC_BOOL super_download_vec_ipaddr_cstr(const UINT32 super_md_id, const CVECTOR *ipaddr_cstr_vec, const CSTRING *fname, CVECTOR *output_cbytes_vec)
{
    ...
    for(ipaddr_pos = 0; ipaddr_pos < cvector_size(ipaddr_cstr_vec); ipaddr_pos ++)
    {
        ...
        if(NULL_PTR == output_cbytes)
        {
            ...
            cvector_clean_no_lock(output_cbytes_vec, (CVECTOR_DATA_CLEANER)cbytes_free, LOC_SUPER_0073);
            task_mgr_free(task_mgr);
            return (EC_FALSE);
        }
...
EC_BOOL super_backup(const UINT32 super_md_id, const CSTRING *fname)
{
    ...
    if(0 == access((char *)cstring_get_str(fname), F_OK))
    {
        ...
        cmd_line = cstring_new(NULL_PTR, LOC_SUPER_0074);
...
EC_BOOL super_collect_vec_ipaddr_cstr(const UINT32 super_md_id, CVECTOR *ipaddr_cstr_vec)
{
    ...
    task_brd = task_brd_default_get();
    ipaddr_vec = cvector_new(0, MM_UINT32, LOC_SUPER_0075);
...
EC_BOOL super_write_fdata(const UINT32 super_md_id, const CSTRING *fname, const UINT32 offset, const CBYTES *cbytes)
{
    ...
    SUPER_FNODE_CMUTEX_LOCK(super_fnode, LOC_SUPER_0079);
...
EC_BOOL super_read_fdata(const UINT32 super_md_id, const CSTRING *fname, const UINT32 offset, const UINT32 max_len, CBYTES *cbytes)
{
    ...
    read_offset = offset;
    SUPER_FNODE_CMUTEX_LOCK(super_fnode, LOC_SUPER_0084);
    ...
    SUPER_FNODE_CMUTEX_UNLOCK(super_fnode, LOC_SUPER_0086);
...
EC_BOOL super_transfer_vec_ipaddr_cstr(const UINT32 super_md_id, const CSTRING *src_fname, const CVECTOR *ipaddr_cstr_vec, const CSTRING *des_fname, CVECTOR *ret_vec)
{
    ...
    tcid_vec = cvector_new(0, MM_UINT32, LOC_SUPER_0087);
    ...
    cvector_free(tcid_vec, LOC_SUPER_0089);
    return (EC_TRUE);
}
...
EC_BOOL super_say_hello_batch(const UINT32 super_md_id, const UINT32 num, const UINT32 des_tcid, const UINT32 des_rank)
{
    ...
    report_vec  = cvector_new(0, MM_UINT32, LOC_SUPER_0090);
    cstring_vec = cvector_new(0, MM_CSTRING, LOC_SUPER_0091);
    ...
    for(idx = 0; idx < num; idx ++)
    {
        ...
        alloc_static_mem(MM_UINT32, &ret, LOC_SUPER_0092);
        cvector_push(report_vec, (void *)ret);

        cstring = cstring_new(NULL_PTR, LOC_SUPER_0093);
    ...
    for(idx = 0; idx < num; idx ++)
    {
        ...
        free_static_mem(MM_UINT32, ret, LOC_SUPER_0094);

        cvector_set(cstring_vec, idx, NULL_PTR);
        cstring_free(cstring);
    } 

    cvector_free(report_vec, LOC_SUPER_0095);
    cvector_free(cstring_vec, LOC_SUPER_0096);
...
EC_BOOL super_say_hello_loop0(const UINT32 super_md_id, const UINT32 loops, const UINT32 des_tcid, const UINT32 des_rank)
{
    ...
    for(count = 0; count < loops; count ++)
    {
        ...
        cstring = cstring_new(NULL_PTR, LOC_SUPER_0097);
...
EC_BOOL super_load_data(const UINT32 super_md_id)
{
    ...
    SUPER_MD_OBJ_ZONE(super_md) = cvector_new(obj_zone_size, MM_CVECTOR, LOC_SUPER_0098);
    if(NULL_PTR == SUPER_MD_OBJ_ZONE(super_md))
    {
    ...
    for(obj_idx = 0; obj_idx < obj_zone_size; obj_idx ++)
    {
        ...
        obj_vec = cvector_new(obj_data_num, MM_UINT32, LOC_SUPER_0099);
        if(NULL_PTR == obj_vec)
        {
            EC_BOOL ret;
            ...
            cvector_loop(SUPER_MD_OBJ_ZONE(super_md),
                        ...
                        NULL_PTR,
                        LOC_SUPER_0100);
            cvector_free(SUPER_MD_OBJ_ZONE(super_md), LOC_SUPER_0101);         
            return (EC_FALSE);
        }
...
EC_BOOL super_get_data_vec(const UINT32 super_md_id, const CVECTOR *obj_id_vec, CVECTOR *obj_data_vec)
{
    ...
    for(obj_id_pos = 0; obj_id_pos < cvector_size(obj_id_vec); obj_id_pos ++)
    {
        ...
        obj_data = cvector_new(0, MM_UINT32, LOC_SUPER_0102);
        if(NULL_PTR == obj_data)
        {
            ...
            cvector_loop_no_lock(obj_data_vec,
                        ...
                        NULL_PTR,
                        LOC_SUPER_0103);
            cvector_clean_no_lock(obj_data_vec, NULL_PTR, LOC_SUPER_0104);
            return (EC_FALSE);
        }
...
EC_BOOL super_cond_wait(const UINT32 super_md_id, const UINT32 tag, const CSTRING *key, const UINT32 timeout_msec)
{
    ...
    croutine_cond_reserve(SUPER_CCOND_COND(super_ccond_inserted), 1, LOC_SUPER_0105);
    if(do_log(SEC_0117_SUPER, 9))
    {
    ...
    if(EC_TIMEOUT == croutine_cond_wait(SUPER_CCOND_COND(super_ccond_inserted), LOC_SUPER_0106))
    {
        if(do_log(SEC_0117_SUPER, 9))
        {
...
EC_BOOL super_cond_wakeup(const UINT32 super_md_id, const UINT32 tag, const CSTRING *key)
{
    ...
    croutine_cond_release_all(SUPER_CCOND_COND(super_ccond_searched), LOC_SUPER_0107);
...
static EC_BOOL __super_http_store_after_ddir(const CHTTP_STORE *chttp_store, const CSTRING *path)
{
    ...
    {
        dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "error:__super_http_store_after_ddir: no crfsmon started\n");
        return (EC_FALSE);
    }
    ...
    if(0 == num)
    {
        dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "error:__super_http_store_after_ddir: store is empty\n");
        return (EC_FALSE);
    }
    ...
    for(pos = 0; pos < num; pos ++)
    {
        ...
        {
            dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] __super_http_store_after_ddir: delete '%.*s' skip rfs %s which is not up\n",
                    CSTRING_LEN(path), CSTRING_STR(path),
    ...
    dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] __super_http_store_after_ddir: delete '%.*s' done\n",
                    CSTRING_LEN(path), CSTRING_STR(path));
...
EC_BOOL super_http_store_after_ddir(const UINT32 super_md_id, const UINT32 tcid, const UINT32 store_srv_ipaddr, const UINT32 store_srv_port, const CSTRING *path, const CBYTES *cbytes, const CSTRING *auth_token, const CHTTP_STORE *chttp_store)
{
    ...
    dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] super_http_store_after_ddir: delete '%.*s' done\n",
    ...
    {
        dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "error:super_http_store_after_ddir: delete '%.*s' done\n",
                        CSTRING_LEN(path), CSTRING_STR(path));
        return (EC_FALSE); 
    }
...
static EC_BOOL __super_unlock_over_bgn(const UINT32 super_md_id, const UINT32 tcid, const UINT32 store_srv_ipaddr, const UINT32 store_srv_port, const CSTRING *path, const CSTRING *auth_token)
{
    ...
    task_p2p(CMPI_ANY_MODI, TASK_DEFAULT_LIVE, TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP,
             &mod_node,
             &ret,
             FI_crfs_file_unlock, ERR_MODULE_ID, path, auth_token);
...
static EC_BOOL __super_read_data_e(const UINT32 super_md_id, const UINT32 store_srv_ipaddr, const UINT32 store_srv_port, const CSTRING *path, const UINT32 store_offset, const UINT32 store_size, CBYTES *cbytes)
{
    ...
    dbg_log(SEC_0149_CHTTP, 1)(LOGSTDOUT, "[DEBUG] __super_read_data_e: read '%.*s' from %s:%ld start \n",
                        CSTRING_LEN(path), CSTRING_STR(path),
                        c_word_to_ipv4(store_srv_ipaddr), store_srv_port);
    ...
    if(EC_FALSE == chttp_request_basic(&chttp_req, NULL_PTR, &chttp_rsp, NULL_PTR))
    {
        dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "error:__super_read_data_e: read '%.*s' on %s:%ld failed\n",
                        CSTRING_LEN(path), CSTRING_STR(path),
                        c_word_to_ipv4(store_srv_ipaddr), store_srv_port);
    ...
    dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] __super_read_data_e: read '%.*s' on %s:%ld back\n",
                    CSTRING_LEN(path), CSTRING_STR(path),
                    c_word_to_ipv4(store_srv_ipaddr), store_srv_port); 

    if(CHTTP_OK != CHTTP_RSP_STATUS(&chttp_rsp))
    {
        dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "error:__super_read_data_e: read '%.*s' on %s:%ld => status %u\n",
                        CSTRING_LEN(path), CSTRING_STR(path),
                        c_word_to_ipv4(store_srv_ipaddr), store_srv_port,
                        CHTTP_RSP_STATUS(&chttp_rsp));
    ...
    dbg_log(SEC_0149_CHTTP, 1)(LOGSTDOUT, "[DEBUG] __super_read_data_e: read '%.*s' on %s:%ld => OK\n",
                    CSTRING_LEN(path), CSTRING_STR(path),
                    c_word_to_ipv4(store_srv_ipaddr), store_srv_port); 
...
static EC_BOOL __super_read_data(const UINT32 super_md_id, const UINT32 store_srv_ipaddr, const UINT32 store_srv_port, const CSTRING *path, CBYTES *cbytes)
{
    ...
    dbg_log(SEC_0149_CHTTP, 1)(LOGSTDOUT, "[DEBUG] __super_read_data: read '%.*s' from %s:%ld start \n",
                        CSTRING_LEN(path), CSTRING_STR(path),
                        c_word_to_ipv4(store_srv_ipaddr), store_srv_port);
    ...
    if(EC_FALSE == chttp_request_basic(&chttp_req, NULL_PTR, &chttp_rsp, NULL_PTR))
    {
        dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "error:__super_read_data: read '%.*s' on %s:%ld failed\n",
                        CSTRING_LEN(path), CSTRING_STR(path),
                        c_word_to_ipv4(store_srv_ipaddr), store_srv_port);
    ...
    dbg_log(SEC_0149_CHTTP, 9)(LOGSTDOUT, "[DEBUG] __super_read_data: read '%.*s' on %s:%ld back\n",
                    CSTRING_LEN(path), CSTRING_STR(path),
                    c_word_to_ipv4(store_srv_ipaddr), store_srv_port); 

    if(CHTTP_OK != CHTTP_RSP_STATUS(&chttp_rsp))
    {
        dbg_log(SEC_0149_CHTTP, 0)(LOGSTDOUT, "error:__super_read_data: read '%.*s' on %s:%ld => status %u\n",
                        CSTRING_LEN(path), CSTRING_STR(path),
                        c_word_to_ipv4(store_srv_ipaddr), store_srv_port,
                        CHTTP_RSP_STATUS(&chttp_rsp));
    ...
    dbg_log(SEC_0149_CHTTP, 1)(LOGSTDOUT, "[DEBUG] __super_read_data: read '%.*s' on %s:%ld => OK\n",
                    CSTRING_LEN(path), CSTRING_STR(path),
                    c_word_to_ipv4(store_srv_ipaddr), store_srv_port); 
...
EC_BOOL super_renew_headers(const UINT32 super_md_id, const UINT32 store_srv_tcid, const UINT32 store_srv_ipaddr, const UINT32 store_srv_port, const CSTRING *path, const CSTRKV_MGR *cstrkv_mgr, const CSTRING *auth_token)
{
    ...
    chttp_req_add_header(&chttp_req_t, (const char *)"renew-num", (char *)c_uint32_to_str(idx));
