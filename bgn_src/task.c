...
#if (SWITCH_OFF == NGX_BGN_SWITCH)
        if(EC_FALSE == tasks_srv_start(tasks_cfg))
        {
...
EC_BOOL task_brd_start_crfsngx_srv(TASK_BRD *task_brd, const UINT32 crfs_md_id, const UINT32 crfsngx_srv_ipaddr, const UINT32 crfsngx_srv_port)
{
    ...
    TASK_BRD_CSRV(task_brd) = csrv_start(crfsngx_srv_ipaddr, crfsngx_srv_port, crfs_md_id,
                                        ...
                                        (CSRV_RD_HANDLER_FUNC      )crfsngx_recv_on_csocket_cnode,
                                        (CSRV_WR_HANDLER_FUNC      )crfsngx_send_on_csocket_cnode,
                                        (CSRV_TIMEOUT_HANDLER_FUNC )csocket_cnode_close,
                                        (CSRV_COMPLETE_HANDLER_FUNC)csocket_cnode_close,
                                        (CSRV_CLOSE_HANDLER_FUNC   )csocket_cnode_close);
    if(NULL_PTR == TASK_BRD_CSRV(task_brd))
    {
...
{    
    ...
    TASK_BRD_CSRV(task_brd) = chttp_srv_start(http_srv_ipaddr, http_srv_port, CMPI_ANY_MODI,
                                (uint32_t)CRFSHTTP_SOCKET_TIMEOUT_NSEC,
                                (CSRV_INIT_CSOCKET_CNODE)chttp_csocket_cnode_init,
                                (CSRV_ADD_CSOCKET_CNODE)NULL_PTR,
                                (CSRV_DEL_CSOCKET_CNODE)NULL_PTR,
                                (CSRV_RD_HANDLER_FUNC      )chttp_csocket_cnode_recv_req,
                                (CSRV_WR_HANDLER_FUNC      )chttp_csocket_cnode_send_rsp,
                                (CSRV_TIMEOUT_HANDLER_FUNC )chttp_csocket_cnode_force_close,
                                (CSRV_COMPLETE_HANDLER_FUNC)chttp_csocket_cnode_close,
                                (CSRV_CLOSE_HANDLER_FUNC   )chttp_csocket_cnode_force_close);
    if(NULL_PTR == TASK_BRD_CSRV(task_brd))
    {
...
EC_BOOL task_brd_start_https_srv(TASK_BRD *task_brd, const UINT32 https_srv_ipaddr, const UINT32 https_srv_port)
{
    ...
    TASK_BRD_CSRV(task_brd) = chttps_srv_start(https_srv_ipaddr, https_srv_port, CMPI_ANY_MODI,
                                        (uint32_t)CRFSHTTP_SOCKET_TIMEOUT_NSEC,
                                        (CSRV_INIT_CSOCKET_CNODE)chttps_csocket_cnode_init,
                                        (CSRV_ADD_CSOCKET_CNODE)NULL_PTR,
                                        (CSRV_DEL_CSOCKET_CNODE)NULL_PTR,
                                        (CSRV_RD_HANDLER_FUNC      )chttps_csocket_cnode_recv_req,
                                        (CSRV_WR_HANDLER_FUNC      )chttps_csocket_cnode_send_rsp,
                                        (CSRV_TIMEOUT_HANDLER_FUNC )chttps_csocket_cnode_force_close,
                                        (CSRV_COMPLETE_HANDLER_FUNC)chttps_csocket_cnode_close,
                                        (CSRV_CLOSE_HANDLER_FUNC   )chttps_csocket_cnode_force_close);
    if(NULL_PTR == TASK_BRD_CSRV(task_brd))
    {
...
EC_BOOL task_brd_default_start_cextsrv(const UINT32 srv_ipaddr, const UINT32 srv_port, const UINT32 thread_num)
{
    ...
    for(thread_pos = 0; thread_pos < thread_num; thread_pos ++)
    {
        ...
        CROUTINE_NODE_COND_RELEASE(croutine_node, LOC_TASK_0207);
    }
...
EC_BOOL task_brd_need_slow_down(TASK_BRD *task_brd, LOG *log, UINT32 level)
{
    ...
    //||  0 < (csocket_cnode_num = csocket_cnode_stat_num())/*Feb 4, 2017*/
...
EC_BOOL do_slave_enhanced(TASK_BRD *task_brd)
{
...
#endif/*(SWITCH_OFF == NGX_BGN_SWITCH)*/
    {
        ...
        else
        {
            ...
            {
                g_do_slave_usleep_counter ++;
                c_usleep(TASK_SLOW_DOWN_MSEC, LOC_TASK_0228);
                /*if slow_down happen, update task_brd time*/
                task_brd_update_time(task_brd);
            }
        }
...
TASK_MGR * task_new(const MOD_MGR *mod_mgr, const UINT32 task_prio, const UINT32 task_need_rsp_flag, const UINT32 task_need_rsp_num)
{
    ...
    alloc_static_mem(MM_TASK_MGR, &task_mgr, LOC_TASK_0234);
