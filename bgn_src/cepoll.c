...
EC_BOOL cepoll_set_complete(CEPOLL *cepoll, const int sockfd, CEPOLL_EVENT_HANDLER complete_handler, void *arg)
{
    CEPOLL_NODE *cepoll_node;
    ...
    dbg_log(SEC_0072_CEPOLL, 9)(LOGSTDOUT, "[DEBUG] cepoll_set_complete: sockfd %d set complete (func %p, arg %p)\n",
                    sockfd, complete_handler, arg);
    return (EC_TRUE);
}
...
EC_BOOL cepoll_set_shutdown(CEPOLL *cepoll, const int sockfd, CEPOLL_EVENT_HANDLER shutdown_handler, void *arg)
{
    ...
    dbg_log(SEC_0072_CEPOLL, 9)(LOGSTDOUT, "[DEBUG] cepoll_set_shutdown: sockfd %d set shutdown (func %p, arg %p)\n",
                    sockfd, shutdown_handler, arg);
    return (EC_TRUE);
}
...
EC_BOOL cepoll_set_timeout(CEPOLL *cepoll, const int sockfd, const uint32_t timeout_nsec, CEPOLL_EVENT_HANDLER timeout_handler, void *arg)
{
    ...
    dbg_log(SEC_0072_CEPOLL, 9)(LOGSTDOUT, "[DEBUG] cepoll_set_timeout: sockfd %d, next %d, timeout %d, handler %p, arg %p done\n",
                       sockfd,
...
EC_BOOL cepoll_set_loop_handler(CEPOLL *cepoll, CEPOLL_LOOP_HANDLER handler, void *arg)
{
    ...
    dbg_log(SEC_0072_CEPOLL, 5)(LOGSTDOUT, "[DEBUG] cepoll_set_loop_handler: loop handler set done\n");
    return (EC_TRUE);
}
...
EC_BOOL cepoll_handle(CEPOLL *cepoll,  const int sockfd, const uint32_t events, CEPOLL_NODE  *cepoll_node)
{
    ...
    if((events & CEPOLL_IN) && (NULL_PTR != CEPOLL_NODE_RD_HANDLER(cepoll_node)))
    {
        ...
        if(EC_DONE == ret)
        {
            dbg_log(SEC_0072_CEPOLL, 9)(LOGSTDOUT, "[DEBUG] cepoll_handle: sockfd %d read done\n", sockfd);
            return (EC_DONE);
        }
    }
    ...
    if((events & CEPOLL_OUT) && (NULL_PTR != CEPOLL_NODE_WR_HANDLER(cepoll_node)))
    {
        ...
        {
            dbg_log(SEC_0072_CEPOLL, 9)(LOGSTDOUT, "[DEBUG] cepoll_handle: sockfd %d write done\n", sockfd);
            return (EC_DONE);
        }
    } 
...
EC_BOOL cepoll_wait(CEPOLL *cepoll, int timeout_ms)
{
    ...
    for(sockfd_idx = 0 ; sockfd_idx < sockfd_num; sockfd_idx ++)
    {
        ...
        if(EC_DONE == ret)/*prevent from error reporting*/
        {
            ...
            dbg_log(SEC_0072_CEPOLL, 9)(LOGSTDOUT, "[DEBUG] cepoll_wait: sockfd %d trigger complete and epoll events remove\n", sockfd);  

            handler = CEPOLL_NODE_COMPLETE_HANDLER(cepoll_node);
            arg     = CEPOLL_NODE_COMPLETE_ARG(cepoll_node);

            cepoll_del_all(cepoll, sockfd);/*epoll del before socket closing*/
            cepoll_clear_node(cepoll, sockfd);/*attention: clear node before execute complete handler to avoid override timeout setting*/
