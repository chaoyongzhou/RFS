...
UINT32 crfsmon_start()
{
    ...
    if(ERR_MODULE_ID == crfsmon_md_id)
    {
...
void crfsmon_end(const UINT32 crfsmon_md_id)
{
    ...
    /* free module : */
    //crfsmon_free_module_static_mem(crfsmon_md_id);
...
EC_BOOL crfsmon_crfs_node_add(const UINT32 crfsmon_md_id, const CRFS_NODE *crfs_node)
{
    ...
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:crfsmon_crfs_node_add: not searched tasks cfg of tcid %s\n",
                            c_word_to_ipv4(CRFS_NODE_TCID(crfs_node)));
        return (EC_FALSE);
    }
